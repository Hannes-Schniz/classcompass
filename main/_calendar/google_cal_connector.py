...
class googleCalCon:
    ...
    def checkInsertEvent(self, event, simulate=None, verbose=None):
        ...
        if verbose:
            print(
                f"[VERBOSE] Checking if event exists: {event['summary']} ({event['start']['dateTime']} - {event['end']['dateTime']})"
            )
        if self.eventExists(event, self.events) != False:
            if simulate or verbose:
                print(
                    f"[SIMULATION][VERBOSE] Event already exists and would be skipped: {event['summary']}"
                )
            return
    ...
    def sendEvent(self, event):
        if self.simulate:
            ...
            self.service.events().insert(
                calendarId=self.env["calendarID"], body=event
            ).execute()
    ...
    def getEntries(self, weeks):
        if self.verbose:
            print(
                f"[VERBOSE] Fetching events for {weeks} weeks ahead from calendar {self.env['calendarID']}"
            )
        ...
        if weeks == None:
            events_result = (
                ...
                .list(
                    ...
                    ),  # Important: Use the timezone string
                )
                .execute()
            )
        else:
    ...
    def eventExists(self, event, eventList, ignorePrefix=False, ignoreEnd=False):
        ...
        for existing_event in eventList:
            ...
            if ignorePrefix and re.compile(f".*{summary}").match(
            ...
            if summarySame and ex_start == new_start and endSame:
                if self.verbose:
                    print(
                        f"[VERBOSE] Found existing event: {summary} ({new_start} - {new_end})"
                    )
            ...
            if dt.tzinfo is None:
                if self.target_timezone:
                    ...
                    dt = pytz.utc.localize(
                        dt
                    )  # assume UTC if no timezone and no target timezone
    ...
    def eql(self, eventOne, eventTwo):
        ...
        if not self.sameDatetime(
            eventOne["start"]["dateTime"], eventTwo["start"]["dateTime"]
        ):
            return False

        if not self.sameDatetime(
            eventOne["end"]["dateTime"], eventTwo["end"]["dateTime"]
        ):
            return False
    ...
    def sameDatetime(self, datetime, newTime):
        ...
        if (
            int(newSplit[1].split(":")[0]) != timeWithOffset
            and int(newSplit[1].split(":")[1]) != split[1].split(":")[1]
        ):
                ...
                if not self.simulate:
                    try:
                        self.service.events().delete(
                            calendarId=self.env["calendarID"], eventId=existing["id"]
                        ).execute()
    ...
    def removeEvents(self, verbose=None):
        ...
        if self.simulate:
            print(
                f"[SIMULATION] The following events would be deleted from calendar {self.env['calendarID']}:"
            )
            if not events:
        ...
        for event in events:
            ...
            try:
                self.service.events().delete(
                    calendarId=self.env["calendarID"], eventId=event_id
                ).execute()
                if self.verbose:
