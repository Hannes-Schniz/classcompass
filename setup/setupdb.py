...
def main():
    ...
    try:
        ...
        if os.environ.get(dbParams.DBPATHVAR.value):
            ...
            print(
                f"[{logLevel.INFO.value}] Using default database path (set DB_PATH or DATABASE_PATH to override)"
            )
        ...
        if os.environ.get(dbParams.SQLDIRVAR.value):
            ...
            print(
                f"[{logLevel.INFO.value}] Using default SQL directory (set SQL_DIR to override)"
            )

        sql_files = [
            (
                os.path.join(sql_dir, dbParams.CREATECLASSESSQLFILE.value),
                "Classes table creation",
            ),
            (
                os.path.join(sql_dir, dbParams.CREATEDIFFSQLFILE.value),
                "Diff table creation",
            ),
            (
                os.path.join(sql_dir, dbParams.CREATENOTIFICATIONFILE.value),
