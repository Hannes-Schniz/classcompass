...
from constants import dbParams, files
...
DEFAULT_CONFIG = {
    "classID": "0",
    "color-scheme": {"primary": "1", "cancelled": "11", "changed": "5", "exam": "10"},
    "weeksAhead": 3,
    "maintenance": False,
    "showCancelled": False,
    "showChanges": False,
...
def get_database_stats():
    ...
    try:
        ...
        try:
            ...
            cur.execute(
                "SELECT MIN(date) as min_date, MAX(date) as max_date FROM classes"
            )
            ...
            if latest_batch:
                cur.execute(
                    "SELECT COUNT(*) as count FROM classes WHERE batchID = ?",
                    (latest_batch,),
                )
...
def index():
    ...
    return render_template(
        "dashboard.html", config=config, environment=environment, db_stats=db_stats
    )
